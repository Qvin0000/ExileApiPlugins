    ...
    public class AdvancedTooltip : BaseSettingsPlugin<AdvancedTooltipSettings>
    {
        ...
        public override void Render() {
            ...
            if (mods != null)
            {
                ...
                Graphics.DrawText(string.Concat(Enumerable.Repeat(symbol, t2)), startPosition.Translate(t1 * 14), Settings.ItemMods.T2Color/*,"DFPT_B5_POE:15"*/);
        ...
        private Vector2 DrawMod(ModValue item, Vector2 position) {
            ...
            if (item.AffixType != ModType.Unique)
            {
                if (item.TotalTiers > 0)
                {
                    ...
                    {
                        case ModType.Prefix:
                            nextLine = Graphics.DrawText(affix, position.Translate(x, 0), settings.PrefixColor);
                            if (!TColors.TryGetValue(item.Tier, out TColor)) TColor = settings.PrefixColor;

                            break;
                        case ModType.Suffix:
                            nextLine = Graphics.DrawText(affix, position.Translate(x, 0), settings.SuffixColor);
                ...
                var textSize = Graphics.DrawText(item.AffixText, position.Translate(x + nextLine.X, 0), TColor);
...

            {
                for (var i = 0; i < 4; i++)
                {
                    ...
                    if (item.AffixType == ModType.Unique)
                    {
                        txSize = Graphics.DrawText(statText, position.Translate(x + 30, 0), Color.Gainsboro, FontAlign.Right);
                        var drawText = Graphics.DrawText(line2, position.Translate(x + 40, 0), Color.Gainsboro);
                    }
                    else
                    {
                        txSize = Graphics.DrawText(statText, position.Translate(x, 0), Color.Gainsboro, FontAlign.Right);
                        var drawText = Graphics.DrawText(line2, position.Translate(+40, 0), Color.Gainsboro);
                    }
        ...
        private void DrawWeaponDps(RectangleF clientRect) {
            ...
            var pDpsSize = pDps > 0 ? Graphics.DrawText(pDps.ToString("#.#"), textPosition, FontAlign.Right) : System.Numerics.Vector2.Zero;
            var eDpsSize = eDps > 0
